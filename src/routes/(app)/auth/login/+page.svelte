...
let showusernotfoundbanner = false;
...
<svelte:head>
  <title>Webstackpros | Authentication | login to continue</title>
</svelte:head>
...
<section class="bg-white dark:bg-gray-900">
    <div class="lg:grid lg:min-h-screen lg:grid-cols-12">
      ...
      <!-- user not found banner -->
        ...
        {#if showusernotfoundbanner}
        <div role="alert" class="rounded border-s-4 border-red-500 bg-red-50 p-4">
          <strong class="block font-medium text-red-800"> User with the email doesnot exists </strong>
        
          <p class="mt-2 text-sm text-red-700">
          No user hasbeen found with the email id
          </p>
          <button
          class="text-gray-500 transition hover:text-gray-600"
          on:click={() => (showusernotfoundbanner = false)}
        >
          <span class="sr-only">Dismiss popup</span>
        ...
        <!-- password doesnot match banner -->
        {#if passwordnotmatch}
        <div role="alert" class="rounded border-s-4 border-red-500 bg-red-50 p-4">
          <strong class="block font-medium text-red-800"> Password wrong</strong>
        
          <p class="mt-2 text-sm text-red-700">
         Entered password doesnot match please try again or reset password
          </p>
          <button
          class="text-gray-500 transition hover:text-gray-600"
          on:click={() => (passwordnotmatch = false)}
        >
          <span class="sr-only">Dismiss popup</span>
          ...
          <form action="#" method="POST" class="mt-8 grid grid-cols-6 gap-6" use:enhance>
            ...
            {/if}
              ...
              <p class="mt-4 text-sm text-gray-500 sm:mt-0 dark:text-gray-400">
                ...
                <a href="/auth/signup" class="text-gray-700 underline dark:text-gray-200">Register here</a>.
              </p>
            </div>
          </form>
        </div>
